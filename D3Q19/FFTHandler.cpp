...
struct FFTHandler {
    ...
    static void initialize() {
        real_data.resize(Constants::local_alloc_ps);
        ...
        forward_plan = fftw_mpi_plan_dft_r2c_3d(Constants::N, Constants::N, Constants::N, &real_data[0], reinterpret_cast<fftw_complex*>(&complex_data[0]), MPI_COMM_WORLD, FFTW_MEASURE | FFTW_MPI_TRANSPOSED_OUT);
        inverse_plan = fftw_mpi_plan_dft_c2r_3d(Constants::N, Constants::N, Constants::N, reinterpret_cast<fftw_complex*>(&complex_data[0]), &real_data[0], MPI_COMM_WORLD, FFTW_MEASURE | FFTW_MPI_TRANSPOSED_IN);
    }
    ...
    static void execute_bkwd(valarray<complex<double>>& input, valarray<double>& output) {
        ...
        for (int i = 0; i < Constants::local_alloc_ps; i++) {
            output[i] = real_data[i] / (Constants::N * Constants::N * Constants::N);
        }
        ...
        for (int i = 0; i < Constants::local_N; i++) {
            for (int j = 0; j < Constants::N; j++) {
                for (int k = 0; k < Constants::N; k++) {
                    ...
                    output[index + Constants::local_alloc_ps * offset_index] = real_data[index] / (Constants::N * Constants::N * Constants::N);
                }
            }
        }
    }
