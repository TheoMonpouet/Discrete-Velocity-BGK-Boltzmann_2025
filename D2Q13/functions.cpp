...
void u2geq_hat(valarray<complex<double>>& geqh) {   
    ...
    for (int i = 0; i < Constants::local_alloc_fs; i++) {
        ...
        geqh[i + 2*offset]  = (Temp::rhoh[i] + C2*Temp::u2h[i]                    - Temp::uabsh[i] + C4 * Temp::u22h[i]                                      ) * Constants::weights[2];
        geqh[i + 3*offset]  = (Temp::rhoh[i] - C2*Temp::u1h[i]                    - Temp::uabsh[i] + C4 * Temp::u11h[i]                                      ) * Constants::weights[3];
        ...
        geqh[i + 9*offset]  = (Temp::rhoh[i] + C2*2*Temp::u1h[i]                  - Temp::uabsh[i]                      + C4*4*Temp::u11h[i]                 ) * Constants::weights[9];
        geqh[i + 10*offset] = (Temp::rhoh[i] + C2*2*Temp::u2h[i]                  - Temp::uabsh[i]                      + C4*4*Temp::u22h[i]                 ) * Constants::weights[10];
        geqh[i + 11*offset] = (Temp::rhoh[i] - C2*2*Temp::u1h[i]                  - Temp::uabsh[i]                      + C4*4*Temp::u11h[i]                 ) * Constants::weights[11];
        geqh[i + 12*offset] = (Temp::rhoh[i] - C2*2*Temp::u2h[i]                  - Temp::uabsh[i]                      + C4*4*Temp::u22h[i]                 ) * Constants::weights[12];   
    }
}
...
void rhou1u2(const valarray<complex<double>>& ghat) {
    ...
    for (size_t i = 0; i < Constants::local_alloc_fs; i++) {
        Temp::u1h[i] = ghat[i + 1 * offset] - ghat[i + 3 * offset] + ghat[i + 5 * offset] - ghat[i + 6 * offset]
                     - ghat[i + 7 * offset] + ghat[i + 8 * offset] + 2.0*ghat[i + 9 * offset] - 2.0*ghat[i + 11 * offset];

        Temp::u2h[i] = ghat[i + 2 * offset] - ghat[i + 4 * offset] + ghat[i + 5 * offset] + ghat[i + 6 * offset]
                     - ghat[i + 7 * offset] - ghat[i + 8 * offset] + 2.0*ghat[i + 10 * offset] - 2.0*ghat[i + 12 * offset];
    }
